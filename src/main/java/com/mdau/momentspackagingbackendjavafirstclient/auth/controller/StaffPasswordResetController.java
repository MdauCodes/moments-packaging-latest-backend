...
public class StaffPasswordResetController {
    ...
    // For production scale this should be a DB table, but fits current scale
    ...
    public ResponseEntity<Map<String, String>> forgotPassword(
        ...
        rateLimitConfig.checkLogin(request); // reuse login rate limit

        String email = body.getOrDefault("email", "").trim().toLowerCase();

        userRepository.findByEmailAndDeletedFalse(email).ifPresent(user -> {
            if (!Boolean.TRUE.equals(user.getIsStaff())) return; // customers use OTP flow
    ...
    public ResponseEntity<Map<String, String>> changePassword(
            ...
            com.mdau.momentspackagingbackendjavafirstclient.user.entity.User principal) {
